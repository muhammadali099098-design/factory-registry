...
async function writeToGoogle(patch) {
  ...
  const sid = encodeURIComponent(SPREADSHEET_ID);
  // Сначала очищаем каждый лист целиком, чтобы не оставались старые строки
  for (const name of toClear) {
    await c.request({ url: `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${encodeURIComponent(name)}:clear`, method: "POST" });
  }
  await c.request({ url: `https://sheets.googleapis.com/v4/spreadsheets/${sid}/values:batchUpdate`, method: "POST", data: { valueInputOption: "RAW", data } });
}
